      ...
      <AnimatePresence mode="wait">
        ...
        {(status === "idle" || status === "error") && (
          <motion.div key="input" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
            <div className="mb-2 inline-flex items-center px-3 py-1 bg-accent/10 text-[#F0B90B] text-[10px] uppercase font-bold tracking-widest rounded-full">
            ...
            <div className="grid lg:grid-cols-2 gap-8">
              <div
                ...
                className="bg-surface border-2 border-dashed border-[#3A3F45] rounded-2xl p-16 flex flex-col items-center justify-center cursor-pointer hover:border-accent hover:bg-accent/5 transition-all group"
              ...
              >
                ...
                {file ? (
                  ...
                  <>
                    <div className="w-16 h-16 bg-background border border-[#3A3F45] flex items-center justify-center rounded-2xl mb-6 shadow-xl group-hover:scale-110 transition-transform">
                      <UploadCloud className="w-8 h-8 text-secondary group-hover:text-[#F0B90B] transition-colors" />
                    </div>
                    <h3 className="text-xl font-bold text-[#EAECEF] mb-2">Upload Month 2 Statement</h3>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={8}
                  className="w-full p-4 bg-[#1E2026] text-[#EAECEF] border border-[#3A3F45] rounded-xl text-[#EAECEF] placeholder-gray-400 focus:outline-none focus:border-black transition-colors text-sm font-mono resize-none shadow-xl h-full min-h-[250px]"
            ...
            {errorMsg && (
              <div className="mt-8 flex items-center gap-3 p-4 bg-destructive/50/10 border border-destructive/20 rounded-xl text-[#F6465D] font-bold">
          ...
          <motion.div key="processing" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center justify-center py-32 text-center">
            <div className="relative w-24 h-24 mb-8">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                {status === "uploading" ? <FileText className="w-8 h-8 text-[#F0B90B] animate-pulse" /> : <Loader2 className="w-8 h-8 text-[#F0B90B] animate-spin" />}
              </div>
            </div>
            <h2 className="text-3xl font-bold tracking-tight text-[#EAECEF] mb-3">
          ...
          <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
            ...
            <div className="text-center mb-12">
              <div className="w-24 h-24 bg-accent/10 border border-accent/30 rounded-full flex items-center justify-center mx-auto mb-8 relative">
                ...
                <ShieldCheck className="w-12 h-12 text-[#F0B90B] relative z-10" />
              </div>
              <h1 className="text-5xl font-bold tracking-tight mb-4 text-[#EAECEF]">Escrow Unlocked.</h1>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              ...
              <MetricCard label="Month 1 Spend" value={`₹${result.month1_total.toLocaleString()}`} icon={<BarChart3 className="w-5 h-5 text-[#F6465D]" />} color="red" />
              ...
              <MetricCard label="Savings Score" value={`${result.new_savings_score}/100`} icon={<Activity className="w-5 h-5 text-[#F0B90B]" />} color="accent" />
            </div>

            {/* Category Comparison */}
            <div className="bg-[#1E2026] text-[#EAECEF] border border-[#3A3F45] rounded-xl p-6 shadow-xl mb-8">
              <h3 className="text-sm font-bold text-[#EAECEF] uppercase tracking-widest mb-4 flex items-center gap-2">
                <BarChart3 className="w-4 h-4 text-[#848E9C]" /> Category-wise Comparison
              </h3>
                ...
                {Object.entries(result.category_comparison).map(([cat, data]) => (
                  <div key={cat} className="flex items-center gap-4">
                    ...
                    <div className="w-24 text-right">
                      <span className="text-xs font-mono text-[#848E9C]">₹{data.month1}</span>
                      <span className="text-xs text-gray-300 mx-1">→</span>
                      <span className="text-xs font-mono text-green-600 font-bold">₹{data.month2}</span>
                    </div>
                    <div className={`w-16 text-right text-xs font-bold font-mono ${data.change < 0 ? "text-green-600" : "text-[#F6465D]"}`}>
                      {data.change}%
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-4 pt-4 border-t border-gray-100 flex gap-4 text-[10px] font-bold uppercase tracking-widest text-[#848E9C]">
              ...
              <div className="bg-[#1E2026] text-[#EAECEF] border border-[#3A3F45] rounded-xl p-6 shadow-xl">
                ...
                <div className="space-y-2">
                  {result.behavioral_insights.map((insight, i) => (
                    ...
                    >
                      <CheckCircle2 className="w-3.5 h-3.5 text-[#F0B90B] mt-0.5 shrink-0" />
            ...
            <div className="flex flex-col md:flex-row gap-6 mb-12">
              <div className="bg-surface border border-[#3A3F45] rounded-xl p-6 text-left w-full md:w-1/3 flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="font-mono text-2xl text-[#EAECEF] font-bold">{result.streak_updated} Days</div>
                </div>
              </div>
              ...
              {result.blockchain_tx && (
                <div className={`bg-accent/50/10 border-accent/20 border rounded-xl p-6 text-left w-full md:w-2/3`}>
                  ...
                  <div className="bg-[#1E2026] text-[#EAECEF]/60 border border-green-200/40 rounded-lg p-4 mb-4 space-y-1.5">
                    <div className="text-[10px] font-bold text-[#848E9C] uppercase tracking-widest mb-2 flex items-center gap-1.5">
                      <ShieldCheck className="w-3 h-3" /> Data Embedded In Transaction
                    </div>
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                      <span className="text-[#848E9C] font-mono">type</span>
                      <span className="text-gray-700 font-bold font-mono">escrow_unlock</span>
                      <span className="text-[#848E9C] font-mono">status</span>
                      <span className="text-green-600 font-bold font-mono">success</span>
                      <span className="text-[#848E9C] font-mono">reduction</span>
                      <span className="text-green-600 font-bold font-mono">{result.reduction_achieved}%</span>
                      <span className="text-[#848E9C] font-mono">challenge</span>
                      <span className="text-gray-700 font-bold font-mono truncate">onchain_challenge</span>
                    </div>
                    <p className="text-[10px] text-[#848E9C] mt-2 pt-2 border-t border-gray-100">
                      ↑ This JSON was SHA-256 hashed and embedded in the tx <code className="text-[#848E9C]">data</code> field. Click &quot;+ Show more&quot; on Etherscan to see the raw hex.
                    </p>
                  </div>
                  ...
                  {result.blockchain_tx && (
                    <a
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-accent/50 text-white text-xs font-bold rounded-lg transition-colors shadow-xl"
                    >
...
function MetricCard({ label, value, icon, color }: { label: string; value: string; icon: React.ReactNode; color: string }) {
  const borderColor = color === "green" ? "border-green-100" : color === "red" ? "border-red-100" : "border-[#3A3F45]";
  return (
    <div className={`bg-[#1E2026] text-[#EAECEF] border ${borderColor} rounded-xl p-5 shadow-xl`}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-[10px] font-bold text-[#848E9C] uppercase tracking-widest">{label}</span>
        {icon}
      </div>
      <div className="text-2xl font-bold text-[#EAECEF] tracking-tight">{value}</div>
    </div>
  );
}
